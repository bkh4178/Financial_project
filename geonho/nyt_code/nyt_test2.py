...
KEYWORD_DICT: dict[str, list[str]] = {
    ...
    "US High Yield Bond": [
        ...
        "junk bond",
        "high-yield debt",
        "ICE BofAML US HY Constrained Index",
    ],
    "US 10 Year Treasury": [
        "10-year Treasury",
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        ...
        if hits > 1000 or total_pages > PAGE_SPLIT_THRESHOLD:
            ...
            for q_begin, q_end, q_label in quarter_ranges(year):
                ...
                q_rows, q_hits, q_pages = collect_period_rows(keyword, variants, q_begin, q_end)
                time.sleep(SEARCH_SLEEP)
                    ...
                    for month in range(start_month, end_month + 1):
                        ...
                        if m_hits > 1000 or m_pages > PAGE_SPLIT_THRESHOLD:
                            ...
                            for d_begin, d_end in [
                                ...
                                h_rows, _ = collect_period(keyword, variants, d_begin, d_end)
                                add_rows(h_rows)
                                time.sleep(SEARCH_SLEEP)
                        else:
                            add_rows(m_rows)
