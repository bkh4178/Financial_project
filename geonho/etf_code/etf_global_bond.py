...
OUTPUT_PATH = OUTPUT_DIR / "etf_global_bond_close.csv"

SHEET_NAME = "ETF (187)"
HEADER_ROW = 3

# 구분2 기준: 해외채권만 사용
INCLUDE_CATEGORIES = ["해외채권_회사채", "해외채권_종합"]
...
def collect_global_bond_prices(df_universe: pd.DataFrame) -> pd.DataFrame:
    """해외채권 ETF 종가 데이터를 pykrx로 수집한다."""
    collected = []

    print("=" * 60)
    print("해외채권 ETF 종가 수집 시작")
...
if __name__ == "__main__":
    ...
    print("해외채권 ETF 분류 요약")
    print(df_universe[["티커", "ETF명", "구분2", "sector_sub"]].to_string(index=False))
    print()

    df_prices = collect_global_bond_prices(df_universe)
