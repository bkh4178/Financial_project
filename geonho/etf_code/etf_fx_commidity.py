...
OUTPUT_PATH = OUTPUT_DIR / "fx_commodity_etf_close.csv"

SHEET_NAME = "ETF (187)"
HEADER_ROW = 3

# FX 및 원자재는 현재 대상 티커 7개를 직접 지정
TARGET_TICKERS = [
    "A411060",  # ACE KRX금현물
    "A132030",  # KODEX 골드선물(H)
    "A144600",  # KODEX 은선물(H)
    "A292560",  # TIGER 일본엔선물
    "A261240",  # KODEX 미국달러선물
    "A261220",  # KODEX WTI원유선물(H)
    "A271060",  # KODEX 3대농산물선물(H)
]
...
def load_etf_universe(xlsx_path: Path) -> pd.DataFrame:
    ...
    # 현재 프로젝트 대상 7개만 사용
    df = df[df["티커"].isin(TARGET_TICKERS)].copy().reset_index(drop=True)

    # pykrx용 6자리 코드
    df["티커_krx"] = df["티커"].astype(str).str.replace("A", "", regex=False)

    # 메인 섹터와 서브 섹터 러프 매핑
    df["sector_main"] = "real_asset"
    df["sector_sub"] = df["티커"].map({
        "A411060": "gold",
        "A132030": "gold",
        "A144600": "silver",
        "A292560": "fx",
...
def collect_fx_commodity_prices(df_universe: pd.DataFrame) -> pd.DataFrame:
    """FX 및 원자재 ETF 종가 데이터를 pykrx로 수집한다."""
    collected = []

    print("=" * 60)
    print("FX 및 원자재 ETF 종가 수집 시작")
...
if __name__ == "__main__":
    ...
    print("FX 및 원자재 ETF 분류 요약")
    print(df_universe[["티커", "ETF명", "sector_sub"]].to_string(index=False))
    print()

    df_prices = collect_fx_commodity_prices(df_universe)
